...
export class Texfont {

    public texobj: any;
    public tex_width: any;
    public tex_height: any;
    public max_ascent: any;
    public max_descent: any;
    public num_glyphs: any;
    public min_glyph: any;
    public range: any;
    public teximage: any;
    public tgi: any;
    public tgvi: any;
    ...
    LoadTexfont(filename: any, txf: any) {
        let data = 0;
        ...
        loader.load(
            ...
            function (data) {
                if (!this.checkMagicNumber(data)) return;		// abort on fail
                this.swap = this.checkEndianness(data);			// do we need byte-swapping?
                this.readTexfontGlobals(txf, data);
                this.readGlyphMeta(txf, data);
                this.computeGlyphVertexInfo(txf);
                this.fetchTextureBitmap(txf, data);
                return txf;
            },
            // onProgress callback
            function (xhr) {
    ...
    GetStringMetrics(txf: any, str: any) {
        /*
        ...
        for (i = 0; i < str.length; i++) {
            tgvi = this.getTGVI(txf, str.charAt(i));
            w += tgvi.advance;
    ...
    RenderString(txf: any, text: any) {
        ...
        for (let i = 0; i < text.length; i++) {
            retVal = this.RenderGlyph(txf, text.charAt(i));
    ...
    public fp = 0;					// byte pos in file; file starts @ fp = 0;
    public format = 99;			// 0: bytemap; 1: bitmap
    public swap = false;			// false: Big Endian; true: Little Endian - needs byte swapping
    ...
    checkMagicNumber(data) {
        ...
        */
        ...
        var magic = data.slice(this.fp, this.fp += 4, "");
    ...
    checkEndianness(data) {
        ...
        */
        ...
        var endianness = data.slice(this.fp, this.fp += 4, "");
    ...
    readTexfontGlobals(txf, data) {
        ...
        */
        ...
        this.format = this.getUint32Val(data);
        txf.tex_width = this.getUint32Val(data);
        txf.tex_height = this.getUint32Val(data);
        txf.max_ascent = this.getUint32Val(data);
        txf.max_descent = this.getUint32Val(data);
        txf.num_glyphs = this.getUint32Val(data);

        console.log("%cLeaving readTexfontGlobals()", "color: green; font-weight: bold");

    }

    readGlyphMeta(txf: any, data: any) {
        /*
        ...
        for (let i = 0; i < txf.num_glyphs; i++) {
            ...
            tgi.c = this.getUint16Val(data).toString();
            tgi.width = this.getUint8Val(data);
            tgi.height = this.getUint8Val(data);
            tgi.xoffset = this.getInt8Val(data);
            tgi.yoffset = this.getInt8Val(data);
            tgi.advance = this.getInt8Val(data);
            tgi.dummy = this.getInt8Val(data).toString();
            tgi.x = this.getUint16Val(data);
            tgi.y = this.getUint16Val(data);
    ...
    fetchTextureBitmap(txf, data) {
        ...
        switch (this.format) {
            case 0:			// TXF_FORMAT_BYTE	-	1 byte per pixel
                ...
                size = height * width;
                bitmap = data.slice(this.fp, this.fp += size, "");
    ...
    getUint32Val(data) {
        ...
        */
        ...
        var buf = data.slice(this.fp, this.fp += 4, "");
    ...
    getUint16Val(data) {
        ...
        */
        ...
        var buf = data.slice(this.fp, this.fp += 2, "");
        let view = new Uint8Array(buf);
    ...
    getUint8Val(data) {
        ...
        */
        ...
        var buf = data.slice(this.fp, this.fp += 1, "");
        let view = new Uint8Array(buf);
...
class myGridHelper extends LineSegments {
    ...
    constructor(width = 10, descent = 10, ascent = 20, hexcolor = 0x444444) {
